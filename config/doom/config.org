...
** Sending Mail
Using msmtpq for queued sending - emails are queued locally and sent in background.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(map! :leader
      :desc "Remove buffer from workspace" "b r" #'persp-remove-buffer)
#+end_src
...
** Org-roam
Zettelkasten-style note-taking with backlinks.
#+begin_src emacs-lisp
(after! org-roam
    :ensure t
    :init
...
** Vulpea Setup
Vulpea extends org-roam with better note metadata and agenda integration.
This configures capture templates, inbox, and custom agenda views.
...
(use-package! vulpea
  ...
  ;; Inbox file - one per machine
  (defvar vulpea-capture-inbox-file
    (format "inbox-%s.org" (system-name))
    "The path to the inbox file, relative to `org-roam-directory'.")
  (setq vulpea-capture-inbox-file
        (expand-file-name vulpea-capture-inbox-file org-roam-directory))
