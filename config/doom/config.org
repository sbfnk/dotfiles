...
** Completion
Rebind =embark-select= from =C-SPC= (grabbed by yabai) to =C-;= so multi-selection works in vertico.
#+begin_src emacs-lisp
(after! embark
...
Minimal machines skip it outright: the file requires notmuch, and =t= only
covers a missing =email.el=, not a failure inside it.
#+begin_src emacs-lisp
(when sf/doom-full
