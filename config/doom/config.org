...
(after! mu4e
  (setq mu4e-bookmarks
        '(("(maildir:/lshtm/inbox OR maildir:/lshtm/\"Sent Items\") (date:today..now OR flag:unread) " "Work email (today)" ?b)
          ("(maildir:/lshtm/inbox OR maildir:/lshtm/\"Sent Items\") " "Work email" ?B)
...
(setq user-full-name "Sebastian Funk"
      user-mail-address "sebastian.funk@lshtm.ac.uk")
#+end_src
...
(after! notmuch
  (setq notmuch-saved-searches
        '((:name "inbox"      :query "tag:inbox"                    :key "i")
          ...
          ;; Per-account inboxes
          (:name "lshtm"      :query "folder:lshtm/INBOX"           :key "l")
...
(after! notmuch
  ...
  (setq notmuch-fcc-dirs
        '(("sebastian.funk@lshtm.ac.uk" . "lshtm/Sent")
  ...
  ;; Toggle between preferring text/plain and text/html
  (defun sf/notmuch-toggle-html-preference ()
    "Toggle between preferring plain text and HTML in multipart emails."
    (interactive)
    (if (member "text/plain" notmuch-multipart/alternative-discouraged)
        (progn
          (setq notmuch-multipart/alternative-discouraged '("text/html"))
  ...
  (add-hook! 'notmuch-show-mode-hook
    (defun sf/notmuch-show-evil-bindings ()
      ...
      (evil-local-set-key 'normal "H" #'sf/notmuch-toggle-html-preference)
      (evil-local-set-key 'normal "V" #'sf/notmuch-view-in-browser)
      (evil-local-set-key 'normal "q" #'notmuch-bury-or-kill-this-buffer)
      ;; Navigation
      (evil-local-set-key 'normal "gl" #'sf/notmuch-next-link)
      (evil-local-set-key 'normal "gL" #'sf/notmuch-prev-link)
      (evil-local-set-key 'normal "ga" #'sf/notmuch-next-attachment)
      (evil-local-set-key 'normal "gA" #'sf/notmuch-prev-attachment)
      (evil-local-set-key 'normal "go" #'sf/notmuch-open-part-externally)
      (evil-local-set-key 'normal "gx" #'browse-url-at-point)))

  ;; Exclude spam/junk from searches
  (setq notmuch-search-hide-excluded t
        notmuch-excluded-tags '("spam" "junk" "deleted"))

  ;; Dedicated mail workspace
  (defun sf/switch-to-mail-workspace ()
    "Switch to or create the mail workspace."
    (+workspace-switch "mail" t))

  (defun sf/notmuch-in-workspace (orig-fun &rest args)
    "Run notmuch command in dedicated mail workspace."
    (sf/switch-to-mail-workspace)
    (apply orig-fun args))

  (advice-add 'notmuch :around #'sf/notmuch-in-workspace)
  (advice-add 'notmuch-search :around #'sf/notmuch-in-workspace)
  (advice-add 'notmuch-mua-new-mail :around #'sf/notmuch-in-workspace)

  ;; Mail keybindings under SPC e (email)
  (map! :leader
        (:prefix ("e" . "email")
         :desc "Notmuch" "m" #'notmuch
         :desc "Compose" "c" #'notmuch-mua-new-mail
         :desc "Flush queue" "f" #'sf/mail-queue-flush
         :desc "Queue list" "q" #'sf/mail-queue-list
         :desc "View log" "l" (cmd! (find-file "~/.log/msmtp.log")))))
#+end_src
...
** Active Tag for Agenda Files
Files with TODOs get "active" filetag. Agenda only includes active files.
Scans ~/org-roam where vulpea/org-roam notes live.
#+begin_src emacs-lisp
(defvar sf/notes-directory "~/org-roam"
  "Directory containing org-roam/vulpea notes.")

(defun sf/org-buffer-has-todos-p ()
  "Return non-nil if current buffer has any active TODO entries."
  (save-excursion
    (goto-char (point-min))
    (re-search-forward "^\\*+ TODO[ \t\n[]" nil t)))

(defun sf/org-add-active-tag ()
  "Add 'active' to #+filetags if not present."
  (save-excursion
    (goto-char (point-min))
    (cond
     ;; Has filetags line - append active
     ((re-search-forward "^#\\+filetags:.*:\\([^:]*\\):?$" nil t)
      ...
      (unless (save-excursion
        ...
        (if (looking-back ":" 1)
            (insert "active:")
          (insert ":active:"))))
     ;; Has title - add filetags after it
     ...
     (t
      (goto-char (point-min))
      (insert "#+filetags: :active:\n")))))

(defun sf/org-scan-active-all ()
  "Scan all org files and add 'active' tag to those with TODOs."
  (interactive)
    ...
    (dolist (file files)
      (let ((filepath (expand-file-name file sf/notes-directory)))
        (with-current-buffer (find-file-noselect filepath)
          (when (and (sf/org-buffer-has-todos-p)
                     (save-excursion
                       (goto-char (point-min))
                       (not (re-search-forward "^#\\+filetags:.*:active:" nil t))))
            (sf/org-add-active-tag)
            (save-buffer)
            (cl-incf count))
          (unless (buffer-modified-p)
            (kill-buffer)))))
    (message "Added 'active' tag to %d files" count)))
