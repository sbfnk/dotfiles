...
** Profile
Desktop-only packages are gated on the install profile, so a =--minimal=
machine does not build org-roam, mail or social clients. See =profile.el=.
...
* Profile
=profile.el= records whether this machine was installed with =--minimal=;
=init.el= reads it to pick the module list, and the sections below use it to
skip desktop-only setup. Loading it again here is a no-op.
#+begin_src emacs-lisp
(load! "profile" doom-user-dir)
#+end_src
...
:END:
This chapter tangles to =mail-config.el=, which only full machines load, so
anything added here stays off servers without needing its own guard.
...
=notes.el=, which minimal machines skip — they have no =~/org-roam= and no
=:lang org= module, and these blocks evaluate vulpea variables at load time.
#+begin_src emacs-lisp
(when sf/doom-full
